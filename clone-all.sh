...
# Toolchain
git clone -o upstream git://sourceware.org/git/binutils-gdb.git
git clone -o upstream https://gcc.gnu.org/git/gcc.git
git clone -o upstream git://sourceware.org/git/newlib-cygwin.git newlib

# Bristol / Embecosm Embedded Benchmark Suite (BEEBS)
git clone -o embecosm git@github.com:embecosm/riscv-beebs.git beebs

# Emulator
git clone -o upstream https://git.qemu.org/git/qemu.git
